...
export const projects: Project[] = [
  {
    ...
    description:
      "Solo-designed, built, deployed and operated studywithlumi.com end-to-end — product, design and engineering — now serving 220+ students across 3K+ AI interactions. Students sign in with Google, organize their work into classes, and upload their own materials (PDFs, lecture notes, slides, textbooks); the AI tutor answers strictly from that content rather than the open web. Lumi auto-generates scored practice quizzes with full review, one-click flashcard decks, and an SM-2 spaced-repetition queue that resurfaces cards right before they'd be forgotten, plus hands-free voice study, per-session notes and revisitable history. The core engineering problem was serving LLM inference safely and affordably at student-scale traffic: every AI call routes through a Vercel serverless function that verifies the caller's Supabase token, rejects anonymous calls and keeps provider keys entirely server-side, with response streaming and prompt caching that serves 87% of input tokens from cache. Because students upload their own coursework, authorization lives in the database rather than in application code — database-enforced row-level security across all 13 user tables, Google OAuth, hardened SECURITY DEFINER RPCs, signed storage and self-service account deletion — so no query path can return another user's data. Once the web app was stable I extended it to iOS and Android with Capacitor, reusing 95% of the existing TypeScript codebase in native shells with platform-specific auth and storage, and submitting both builds for store review from a single shared repository.",
    stack: [
      "React",
      "TypeScript",
      "Vite",
    ...
    metrics: [
      ...
      { label: "AI interactions", value: "3K+" },
